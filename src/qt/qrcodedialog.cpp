...
{
    QString ret = QString("sabitcoin:%1").arg(address);
    int paramCount = 0;
    ...
    if (ui->chkReqPayment->isChecked())
    {
        ...
        {
            // even if we allow a non SBC unit input in lnReqAmount, we generate the URI with SBC as unit (as defined in BIP21)
            ret += QString("?amount=%1").arg(SabitcoinUnits::format(SabitcoinUnits::SBC, ui->lnReqAmount->value()));
            paramCount++;
        }
        else
        {
