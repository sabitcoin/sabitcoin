...
const string strMessageMagic = "Sabitcoin Signed Message:\n";
...
bool CTxOut::IsDust() const
{
    ...
    // so dust is a txout less than 54 uSBC
...
void ThreadScriptCheck() {
    RenameThread("sabitcoin-scriptch");
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("SabitcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("SabitcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static SabitcoinMiner(CWallet *pwallet)
{
    printf("SabitcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("sabitcoin-miner");
    ...
    try { loop {
        ...
        printf("Running SabitcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("SabitcoinMiner terminated\n");
        throw;
    }
}

void GenerateSabitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&SabitcoinMiner, pwallet));
}
